...
class US_Widget_Login extends US_Widget {
	...
	function widget( $args, $instance ) {
		...
		if ( is_user_logged_in() ) {
			...
			$output .= '<span class="w-profile-name">' . $current_user->display_name . '</span>';
			$output .= '</a>';
			$output .= '<a class="w-profile-link for_logout" href="' . wp_logout_url( esc_url( $instance['logout_redirect'] ) ) . '">' . __( 'Log Out', 'us' ) . '</a>';
			$output .= '</div>';
